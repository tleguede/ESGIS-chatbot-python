...
param(
    ...
    [ValidateSet('bot', 'clean', 'venv', 'install', 'build', 'deploy-local', 'deploy', 'serve', 'test', 'test-endpoint', 'remove-webhook', 'help')]
    [string]$target = "help",
...
function Show-Help {
    ...
    Write-Host "  .\make.ps1 remove-webhook - Remove Telegram webhook"
...
function Remove-Webhook {
    Write-Host "Suppression du webhook Telegram..." -ForegroundColor Green
    
    try {
        $token = $env:TELEGRAM_BOT_TOKEN
        if (-not $token) {
            Write-Host "❌ TELEGRAM_BOT_TOKEN n'est pas défini dans les variables d'environnement" -ForegroundColor Red
            exit 1
        }
        ...
        if ($response.ok) {
            ...
            Write-Host "Le bot est maintenant en mode polling" -ForegroundColor Cyan
        } else {
            Write-Host "❌ Erreur lors de la suppression du webhook: $($response.description)" -ForegroundColor Red
        }
    }
    catch {
        Write-Host "❌ Erreur lors de la suppression du webhook: $_" -ForegroundColor Red
        exit 1
    }
}
